import { NextResponse } from 'next/server';
import { JobProvider, JobStatus, JobType } from '@prisma/client';
import { faceSwap } from '@/lib/ai/modelslab';
import { prisma } from '@/lib/db';
import { getWebhookUrl, isValidUuid } from '@/lib/webhooks/utils';
import { deductCreditsForOperation, refundCredits } from '@/lib/credits';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';
export const maxDuration = 120;

// Upload base64 images to ModelsLab's base64_to_url endpoint
// (same approach used by image edit)
const BASE64_TO_URL_ENDPOINT = 'https://modelslab.com/api/v6/image_editing/base64_to_url';

/**
 * Upload a base64 data URI to ModelsLab and return a hosted URL.
 * Falls back to returning the original value if it's already a URL.
 */
async function uploadBase64Image(dataUri: string, apiKey: string): Promise<string> {
  // Already a URL — no upload needed
  if (/^https?:\/\//i.test(dataUri)) return dataUri;

  // Ensure proper data URI format
  const image = dataUri.startsWith('data:image/')
    ? dataUri
    : `data:image/png;base64,${dataUri}`;

  const response = await fetch(BASE64_TO_URL_ENDPOINT, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ key: apiKey, init_image: image }),
  });

  if (!response.ok) {
    throw new Error(`Image upload failed with status ${response.status}`);
  }

  const data = await response.json();

  // Extract URL from various possible response shapes
  const url =
    (Array.isArray(data.output) ? data.output[0] : data.output) ||
    data.url ||
    data.link ||
    (Array.isArray(data.links) ? data.links[0] : data.links);

  if (!url) {
    throw new Error('Image upload returned no URL');
  }

  return url;
}

/**
 * Get a ModelsLab API key for uploads (using same env vars as modelslab.ts)
 */
function getApiKey(): string {
  // Check numbered keys first, then fallback to single key
  for (let i = 1; i <= 10; i++) {
    const key = process.env[`MODELSLAB_API_KEY_${i}`]?.trim();
    if (key) return key;
  }
  const key = process.env.MODELSLAB_API_KEY?.trim();
  if (key) return key;
  throw new Error('No ModelsLab API key configured');
}

/**
 * POST /api/ai/face-swap
 *
 * Perform face swap using ModelsLab Face Swap API.
 *
 * Headers:
 *   x-vp-user-id: Required user ID
 *
 * Body:
 *   mode: 'single' | 'multi' (required)
 *   initImage: base64 data URI or URL of source face image (required)
 *   targetImage: base64 data URI or URL of target image (required)
 *   referenceImage: base64 data URI or URL of reference face (required for multi mode)
 */
export async function POST(request: Request) {
  // -- Auth --
  const userId = request.headers.get('x-vp-user-id')?.trim() ?? '';
  if (!isValidUuid(userId)) {
    return NextResponse.json(
      { error: 'Unauthorized. Missing or invalid x-vp-user-id header.' },
      { status: 401 }
    );
  }

  // -- Parse body --
  let body: Record<string, unknown>;
  try {
    body = await request.json();
  } catch {
    return NextResponse.json({ error: 'Invalid JSON body.' }, { status: 400 });
  }

  const mode = body.mode === 'multi' ? 'multi' : 'single';
  const rawInitImage = typeof body.initImage === 'string' ? body.initImage.trim() : '';
  const rawTargetImage = typeof body.targetImage === 'string' ? body.targetImage.trim() : '';
  const rawReferenceImage = typeof body.referenceImage === 'string' ? body.referenceImage.trim() : '';

  // -- Validation --
  if (!rawInitImage || !rawTargetImage) {
    return NextResponse.json(
      { error: 'initImage and targetImage are required.' },
      { status: 400 }
    );
  }

  if (mode === 'multi' && !rawReferenceImage) {
    return NextResponse.json(
      { error: 'referenceImage is required for multi-face swap mode.' },
      { status: 400 }
    );
  }

  // Deduct credits before calling the API
  const creditDeduction = await deductCreditsForOperation(userId, 'FACE_SWAP');
  if (!creditDeduction.ok) return creditDeduction.response;

  // -- Upload base64 images to get hosted URLs --
  const apiKey = getApiKey();
  let initImageUrl: string;
  let targetImageUrl: string;
  let referenceImageUrl: string | null = null;

  try {
    // Upload all images in parallel for speed
    const uploads = [
      uploadBase64Image(rawInitImage, apiKey),
      uploadBase64Image(rawTargetImage, apiKey),
    ];
    if (mode === 'multi' && rawReferenceImage) {
      uploads.push(uploadBase64Image(rawReferenceImage, apiKey));
    }

    const results = await Promise.all(uploads);
    initImageUrl = results[0]!;
    targetImageUrl = results[1]!;
    referenceImageUrl = results[2] ?? null;
  } catch (error) {
    console.error('[face-swap] Image upload failed:', error);
    return NextResponse.json(
      { error: 'Failed to upload images.', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }

  // -- Create job for tracking --
  const webhookUrl = getWebhookUrl('modelslab');
  let job = null;

  try {
    job = await prisma.generationJob.create({
      data: {
        userId,
        provider: JobProvider.MODELSLAB,
        jobType: JobType.FACE_SWAP,
        status: JobStatus.PENDING,
        requestData: { mode, initImageUrl, targetImageUrl, referenceImageUrl },
      },
    });
  } catch (error) {
    console.error('[face-swap] Failed to create job:', error);
  }

  // -- Call ModelsLab Face Swap API --
  try {
    const webhookOptions = job
      ? { webhook: webhookUrl, trackId: job.trackId }
      : undefined;

    const result = await faceSwap(
      mode,
      initImageUrl,
      targetImageUrl,
      referenceImageUrl,
      webhookOptions,
      { signal: request.signal }
    );

    // Update job with response
    if (job) {
      const now = new Date();
      const isComplete = result.status === 'success';

      await prisma.generationJob.update({
        where: { id: job.id },
        data: {
          externalId: result.requestId,
          status: isComplete ? JobStatus.COMPLETED : JobStatus.PROCESSING,
          responseData: (result.rawResponse as object) ?? null,
          resultUrl: result.imageUrl ?? null,
          startedAt: now,
          completedAt: isComplete ? now : null,
          requestData: {
            mode,
            initImageUrl,
            targetImageUrl,
            referenceImageUrl,
            apiKeyUsed: result.apiKeyUsed,
            requestId: result.requestId,
          },
        },
      });
    }

    return NextResponse.json({
      success: true,
      imageUrl: result.imageUrl,
      status: result.status,
      job: job
        ? { id: job.id, trackId: job.trackId, webhookEnabled: !!webhookUrl }
        : null,
    });
  } catch (error) {
    console.error('[face-swap] Error:', error);
    refundCredits(userId, creditDeduction.creditsDeducted).catch(() => {});

    // Update job as failed
    if (job) {
      await prisma.generationJob.update({
        where: { id: job.id },
        data: {
          status: JobStatus.FAILED,
          errorMessage: error instanceof Error ? error.message : 'Face swap failed',
          completedAt: new Date(),
        },
      });
    }

    return NextResponse.json(
      {
        error: 'Failed to perform face swap.',
        details: error instanceof Error ? error.message : 'Unknown error',
        job: job ? { id: job.id, trackId: job.trackId } : null,
      },
      { status: 500 }
    );
  }
}
